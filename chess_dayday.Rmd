...
```{r}
 finished_games %>% group_by(winner) %>% 
  summarise(n=n()) %>% ggplot(aes(x="", y=n, fill=winner)) + geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) + scale_fill_manual(values=c("black", "grey", "white")) + theme_minimal()
```
...
finished_games %>% group_by(opening_name) %>% 
  ...
  ggplot()+
  geom_col(aes(x=n,y=opening_name))
...
players <- finished_games %>% 
  filter(winner != "draw") %>% 
  select(winner,black_rating,white_rating,turns,opening_ply) %>% gather(color, rating, black_rating:white_rating, factor_key=TRUE) %>% mutate(win = ifelse((winner == "white" & color == "white_rating") | (winner == "black" & color == "black_rating"), 1, 0))
...
Using the data you just created, show two denisty functions (using geom_density) of ratings on the same graph: one for the winners and one for the losers ignoring the games that end as draw. Are the winners' ratings higher than the losers?Yes

```{r}
players %>% ggplot(aes(x=rating, fill = as.factor(win)))+
  geom_density( alpha=0.5)
      
```


Filter your players data to only include those who won. Do you find a correlation between the plater's rating and the number of turns it takes them to win? Find the correlation coefficient and show this in a scatter plot. Do you see a high correlation?NO

```{r}
players %>% filter(win != 0) %>% 
ggplot(aes(x=rating,y=turns))+
  geom_point(alpha = 0.5)+geom_smooth(method = "lm",se = FALSE)

cor(players$rating,players$turns)
```
...
```{r}
 lm(win~rating,data = players, as.factor(color))
```
